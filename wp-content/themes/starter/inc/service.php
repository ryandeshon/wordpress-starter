...
if ( ! function_exists( 'starter_service_display' ) ) :
	/**
	...
	* @since Starter 0.1
	*/
	function starter_service_display() {
		$output = '';

		// get data value from options
		$enable_content = get_theme_mod( 'starter_service_option', 'disabled' );

		if ( starter_check_section( $enable_content ) ) {
			$headline   = get_option( 'ect_service_title', esc_html__( 'Services', 'starter' ) );
			...
			// Select content
				$output .= starter_post_page_category_service();
			$output .= '
						</div><!-- .service-wrapper -->
...
add_action( 'starter_service', 'starter_service_display', 10 );


if ( ! function_exists( 'starter_post_page_category_service' ) ) :
	/**
	 ...
	 * @since Starter 0.1
	 */
	function starter_post_page_category_service() {
		global $post;

		$quantity   = get_theme_mod( 'starter_service_number', 6 );
		...
		$args = array(
			...
			for ( $i = 1; $i <= $quantity; $i++ ) {
				$post_id = '';
					$post_id = get_theme_mod( 'starter_service_cpt_' . $i );
				if ( $post_id && '' !== $post_id ) {
					// Polylang Support.
		...
		while ( $loop->have_posts() ) {
			...
			$output .= '
				...
				if ( has_post_thumbnail() ) {
					$image = get_the_post_thumbnail( $post->ID, 'starter-featured', array( 'title' => $title_attribute, 'alt' => $title_attribute ) );
				}
				else {
					// Get the first image in page, returns false if there is no image.
					$first_image = starter_get_first_image( $post->ID, 'starter-featured', array( 'title' => $title_attribute, 'alt' => $title_attribute ) );
...
endif; // starter_post_page_category_service